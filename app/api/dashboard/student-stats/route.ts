...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get enrolled courses count (active or in-progress)
    const enrolledCoursesResult = await sql`
      SELECT COUNT(*) as count
      FROM enrollments
      WHERE user_id = ${userId}
        AND status IN ('active', 'in-progress', 'completed') -- Include completed for total courses
    `
    ...
    // Get average progress (still useful for general overview, though not displayed directly for study hours)
    const avgProgressResult = await sql`
      SELECT AVG(progress) as avg_progress
      FROM enrollments
      WHERE user_id = ${userId}
        AND status IN ('active', 'in-progress', 'completed')
    `
    ...
    // Get total investment (keeping this as it's not related to study hours)
    ...
    // Calculate monthly growth (mock for now, keeping as it's not related to study hours)
    ...
    const stats = {
      ...
      totalCertificates: Number(certificatesResult[0]?.count || 0),
    ...
    return NextResponse.json({
      stats: {
        ...
        completedCourses: 0,
        totalInvestment: 0,
        totalCertificates: 0, // Ensure this fallback is also present
        monthlyGrowth: "+0%",
      },
    }, { status: 500 })
  }
  ...
  try {
    ...
    await sql`
      ...
      SET progress = ${progress || 0},
          ...
          updated_at = NOW(),
          completion_date = ${status === "completed" ? "NOW()" : null} -- Changed completed_at to completion_date for consistency
