...
export async function GET(request: NextRequest) {
  ...
  SELECT
    ...
    cert.issue_date,
    cert.instructor_name,
    c.name as course_category, -- *** CHANGED: Select category name from 'categories' table ***
    t.level as course_level,
    t.duration,
    u.full_name as student_name,
    u.email as student_email,
  ...
  JOIN users u ON cert.user_id = u.id
  JOIN enrollments enr ON enr.user_id = cert.user_id AND enr.training_id = cert.training_id
  LEFT JOIN categories c ON t.category_id = c.id -- *** ADDED: Join with categories table ***
  WHERE cert.user_id = ${userId}
  ORDER BY cert.issue_date DESC
`
