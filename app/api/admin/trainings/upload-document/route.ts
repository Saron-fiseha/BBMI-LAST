...
export async function POST(req: NextRequest) {
  try {
    ...
    // Sanitize filename: strip non-ASCII/unsafe characters, keep extension
    ...
    // Insert DB record — keep original name for display, but store the safe URL
    await sql`INSERT INTO documents (training_id, file_name, file_url)
              VALUES (${trainingId}, ${file.name}, ${"/uploads/documents/" + safeFilename})`;

    return NextResponse.json({ success: true, filePath: "/uploads/documents/" + safeFilename });
  } catch (err) {
    console.error(err);
    return NextResponse.json({ error: "Upload failed" }, { status: 500 });
  }
