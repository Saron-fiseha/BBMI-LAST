...
//     try {
//       // Get instructor's sessions with training information
...
//           title: session.title || `Live Session: ${session.training_title || "General Session"}`,
//           description: session.description,
//           training_id: session.training_id?.toString(),
...
//           title, description, training_id, instructor_id, scheduled_at, 
//           duration, meeting_url, status, created_at, updated_at
//         ) VALUES (
//           ${title}, 
//           ${description || null}, 
//           ${training_id === "none" ? null : training_id}, 
//           ${user.id}, 
//           ${scheduled_at}, 
//           ${duration || 60}, 
//           ${meeting_url || null}, 
    ...
    try {
      ...
      const formattedSessions = sessions.map((session: any) => {
        ...
        return {
          ...
          duration: Number(session.duration || 60),
          meeting_url: session.meeting_url,
          max_participants: Number(session.max_participants || 50),
        }
      })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = await request.json()
    const { title, description, training_id, scheduled_at, duration, meeting_url, max_participants } = body
    ...
    try {
      ...
      const result = await sql`
        INSERT INTO sessions (
          title, description, training_id, instructor_id, scheduled_at, 
          duration, meeting_url, status, max_participants, created_at, updated_at
        ) VALUES (
          ${title}, 
          ${description || null}, 
          ${training_id === "none" ? null : training_id}, 
