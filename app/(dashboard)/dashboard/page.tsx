  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (activityResponse.ok) {
        ...
        console.warn("Failed to fetch recent activity:", activityResponse.status, activityResponse.statusText);
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        ...
        <div className="px-6 animate-pulse space-y-4">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {[...Array(3)].map((_, i) => ( // Only 3 cards now
              <div key={i} className="h-32 bg-gray-200 rounded-lg"></div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="space-y-6 px-6 pt-6 pb-8">
      ...
      <div>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3"> {/* Changed to lg:grid-cols-3 */}
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.totalCourses}</div>
              <p className="text-xs text-muted-foreground">{inProgressCoursesCount} in progress</p>
            </CardContent>
          </Card>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
            ...
            <CardContent className="space-y-4">
              {enrolledCourses?.slice(0, 3).map((course) => (
                ...
                >
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center justify-between">
                      <h4 className="font-semibold">{course.name}</h4> {/* Changed to course.name */}
                      <Badge variant="secondary">{course.level}</Badge>
                    </div>
                    <p className="text-sm text-gray-600">Instructor: {course.instructor}</p>
                    <div className="flex items-center gap-4 text-sm text-gray-500">
                      <span>
                        {course.completed_lessons}/{course.total_lessons} lessons {/* Changed property names */}
                      </span>
                      <span>•</span>
                      <span>Next: {course.next_lesson}</span> {/* Changed property name */}
                    </div>
                    <Progress value={course.progress} className="h-2" />
                  </div>
                  <Button size="sm" asChild>
        ...
        <div className="lg:col-span-1"> {/* Added lg:col-span-1 to explicitly place it */}
          <Card>
            <CardHeader>
              <CardTitle>Quick Actions</CardTitle>
              <CardDescription>Common tasks and shortcuts</CardDescription>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2"> {/* Adjusted grid for 2 columns on medium screens */}
                <Button variant="outline" className="h-20 flex-col bg-transparent" asChild>
                  <Link href="/courses">
                    <BookOpen className="h-6 w-6 mb-2" />
                    Browse Courses
