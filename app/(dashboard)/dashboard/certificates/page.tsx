...
interface Certificate {
  id: number
  user_id: number
  training_id: number // Changed from course_id to training_id to match API
  course_title: string
  course_category: string
  course_level: string
  instructor_name: string
  student_name: string
  completion_date: string // Assuming this is present from enrollment
  issue_date: string
  certificate_code: string
  verification_code: string
  grade: string // Assuming this is present from enrollment
  duration: number // Changed from duration_hours to duration to match API
  skills_learned: string[]
  verification_url: string
  progress: number
  total_modules: number
...
interface Category {
  ...
  name: string
  description: string
  course_count: number // This might not be directly available, consider fetching separately or deriving
}
...
export default function StudentCertificatesPage() {
  ...
  useEffect(() => {
    if (user?.id) {
      fetchCertificates()
      fetchCategories() // Also fetch categories when user ID is available
    }
  }, [user?.id])
  ...
  const fetchCertificates = async () => {
    try {
      ...
      if (data.certificates && data.certificates.length > 0) {
        ...
        const uniqueCategoryNames = [...new Set(data.certificates.map((cert: Certificate) => cert.course_category))];
        const levels = [...new Set(data.certificates.map((cert: Certificate) => cert.course_level))];
        const years = [
          ...new Set(data.certificates.map((cert: Certificate) => new Date(cert.issue_date).getFullYear())),
        ];
        const instructors = [...new Set(data.certificates.map((cert: Certificate) => cert.instructor_name))];

        // Map category names to Category objects
        const categoryObjects: Category[] = uniqueCategoryNames.filter(Boolean).map(name => ({
          id: 0, // You might need to derive a real ID or fetch it
          name: name as string, // Assert as string
          description: "", // You might need to derive a real description
          course_count: 0, // You might need to derive a real count
        }));

        setFilterOptions({
          categories: categoryObjects, // Use the mapped Category objects
          levels: levels.filter(Boolean) as string[], // Assert levels as string[]
          years: (years.filter(Boolean) as number[]).sort((a, b) => b - a), // Assert years as number[]
          instructors: instructors.filter(Boolean) as string[], // Assert instructors as string[]
        });
      } else {
        setCertificates([]);
        setFilterOptions({
        ...
        console.log("No certificates found for this user in the database.")
      }
    } catch (error) {
      console.error("Certificates fetch error:", error)
      setCertificates([])
  ...
  const filterCertificates = () => {
    ...
    // Level filter (uncomment if you want to use this)
    if (selectedLevel !== "all") {
      filtered = filtered.filter((cert) => cert.course_level === selectedLevel)
    }
    ...
    // Year filter
    if (selectedYear !== "all") {
      // Ensure the date parsing is robust. issue_date is more reliable for certificate year
      filtered = filtered.filter((cert) => new Date(cert.issue_date).getFullYear().toString() === selectedYear)
    }

    // Instructor filter (uncomment if you want to use this)
    if (selectedInstructor !== "all") {
      filtered = filtered.filter((cert) => cert.instructor_name === selectedInstructor)
    }
  ...
  const generateCertificate = async (courseId: number) => {
    try {
      ...
      const response = await fetch("/api/certificates/generate", {
        ...
        body: JSON.stringify({
          userId: user?.id,
          courseId: courseId, // This is `training_id` in your backend
        }),
      })
            ...
            >
              ...
              <div className="p-4 bg-white">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-500">
                    <Calendar className="h-3 w-3" />
                    <span>{new Date(certificate.issue_date).toLocaleDateString()}</span> {/* Use issue_date */}
                  </div>
                </div>
                  ...
                  <div className="absolute inset-0 flex flex-col justify-center items-center text-center">
                    ...
                    <div className="absolute top-[58%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 max-w-2xl px-4">
                      <p className="text-sm md:text-base lg:text-lg text-blue-900 leading-relaxed">
                        In recognition of exceptional skill in {selectedCertificate.course_title.toLowerCase()},
                        including long-wear techniques, client consultation, and personalized beauty enhancement.
                        Proudly awarded by BBMI on {new Date(selectedCertificate.issue_date).toLocaleDateString()}. {/* Use issue_date here */}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-3 text-sm text-gray-600">
                    ...
                    <div className="flex items-center gap-2">
                      <Clock className="h-4 w-4 text-green-600" />
                      <span>Duration: {selectedCertificate.duration} hours</span> {/* Use duration */}
                    </div>
                    <div className="flex items-center gap-2">
                      <Award className="h-4 w-4 text-purple-600" />
                      <span>Grade: {selectedCertificate.grade}</span>
