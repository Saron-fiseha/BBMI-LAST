...
function mapCertificateResultToCertificateData(dbResult: any): CertificateData | null {
  ...
  return {
    ...
    trainingTitle: dbResult.training_name,
    completionDate: formattedCompletionDate,
    // The actual write path (createCertificateRecord in
    // certificate-generator.ts) inserts into `certificate_number`, not
    // `certificate_code` — the two are separate columns left over from
    // earlier schema history, and certificate_code is never populated by
    // the code path that's actually used (automatic generation on course
    // completion). Reading certificate_code here was producing NULL for
    // every real certificate. certificate_number is selected as an alias
    // below (with a COALESCE fallback to certificate_code, in case any
    // legacy row only has that one populated).
    certificateNumber: dbResult.certificate_number,
    verificationCode: dbResult.verification_code,
    instructorName: dbResult.instructor_name,
    trainingDescription: dbResult.training_description || undefined, // 'description' from 'trainings' table
  };
  ...
  try {
    const result = await sql`
      SELECT
        ...
        c.issue_date,
        c.completion_date,
        COALESCE(c.certificate_number, c.certificate_code) AS certificate_number,
        c.verification_code,
        c.instructor_name,
        c.training_name,
        c.user_name,
