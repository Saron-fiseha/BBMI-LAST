...
async function testModulesFunctionality() {
  ...
  try {
    ...
    sampleModules.forEach((module) => {
      console.log(
        `   - ${module.name} (${module.code}) - ${module.status} - ${module.duration}min`
      );
    });
    ...
    const activeModules =
      await sql`SELECT COUNT(*) as count FROM modules WHERE status = 'active'`;
    const draftModules =
      await sql`SELECT COUNT(*) as count FROM modules WHERE status = 'draft'`;
    const inactiveModules =
      await sql`SELECT COUNT(*) as count FROM modules WHERE status = 'inactive'`;
